...
#define COLOR_FILTER_BLUE_AND_YELLOW 7
#define COLOR_FILTER_DARK_AND_LIGHT_PURPLE 8
#define COLOR_FILTER_RED_AND_GREEN 9
#define COLOR_FILTER_NIGHTTIME 10
...
vec3 applySingleColorFilter(int filterIndex, vec3 color) {
    switch (filterIndex) {
        ...
        case COLOR_FILTER_BLUE_AND_YELLOW:
                return dot(color, vec3(0.2126,0.7152,0.0722)) > 0.4 ? vec3(1,1,0) : vec3(0,0,1);
case COLOR_FILTER_DARK_AND_LIGHT_PURPLE: {
    float lum = dot(color, vec3(0.2126, 0.7152, 0.0722));
    // Dark purple for dark areas, light purple for bright areas
    return lum > 0.4 ? vec3(0.8, 0.6, 1.0) : vec3(0.3, 0.0, 0.5);
}
case COLOR_FILTER_RED_AND_GREEN: {
    float lum = dot(color, vec3(0.2126,0.7152,0.0722));
    return lum > 0.4 ? vec3(1,0.5,0.5) : vec3(0,0.5,0);
}

case COLOR_FILTER_NIGHTTIME: {
    // Darken without desaturating
    float darkenFactor = 0.5; // 0 = black, 1 = original
    return pow(color, vec3(darkenFactor));
}
