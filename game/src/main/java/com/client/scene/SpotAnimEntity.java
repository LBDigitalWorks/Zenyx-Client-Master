    ...
    public void step(int cycle) {
        if (!this.expired) {
            ...
            if (!this.graphics.isCachedModelIdSet()) {
                while (this.frameCycle > this.graphics.frameLengths[this.frame]) {
                    this.frameCycle -= this.graphics.frameLengths[this.frame];
                    ++this.frame;
                    if (this.frame >= this.graphics.frameIds.length) {
                        this.expired = true;
                        break;
                    }
                }
