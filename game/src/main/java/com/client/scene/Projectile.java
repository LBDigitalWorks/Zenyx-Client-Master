        ...
        if (this.sequenceDefinition != null) {
            if (!this.sequenceDefinition.isCachedModelIdSet()) {
                ...
                while (true) {
                    do {
                        do {
                            if (this.frameCycle <= this.sequenceDefinition.frameLengths[this.flow]) {
                                return;
                            }

                            this.frameCycle -= this.sequenceDefinition.frameLengths[this.flow];
                            ++this.flow;
                        } while(this.flow < this.sequenceDefinition.frameIds.length);

                        this.flow -= this.sequenceDefinition.frameCount;
                    } while(this.flow >= 0 && this.flow < this.sequenceDefinition.frameIds.length);
