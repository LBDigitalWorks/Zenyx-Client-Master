...
public final class Player extends Entity implements RSPlayer {
	...
	// Debug flag - set to true to log identity kit/model info when rendering
	public static boolean DEBUG_APPEARANCE = false;
	private static boolean debuggedOnce = false;
	...
	/**
	 * Call this to trigger appearance debug logging on next render.
	 * Use from a developer command like ::debugappearance
	 */
	public static void triggerAppearanceDebug() {
		DEBUG_APPEARANCE = true;
		debuggedOnce = false;
		System.out.println("[DEBUG] Appearance debug enabled - will log on next model render");
	}
	...
	public void updatePlayer(Buffer stream) {
		...
		npcDefinition = null;
		team = 0;
		// Debug: Log equipment values received from server (controlled by DEBUG_APPEARANCE)
		boolean debugEquipment = DEBUG_APPEARANCE && this == Client.instance.localPlayer;
		for (int j = 0; j < 12; j++) {
			int k = stream.readUnsignedByte();
			if (k == 0) {
				equipment[j] = 0;
				if (debugEquipment) {
					System.out.println("  Slot " + j + ": 0 (empty)");
				}
				continue;
			}
			...
			equipment[j] = (k << 8) + i1;
			if (debugEquipment) {
				// Extended kit range: 256-1023 for kits, 1024+ for items
				String type = equipment[j] >= 1024 ? "ITEM " + (equipment[j] - 1024) :
				              equipment[j] >= 256 ? "KIT " + (equipment[j] - 256) : "RAW";
				System.out.println("  Slot " + j + ": " + equipment[j] + " (" + type + ")");
			}
			...
			if (equipment[j] >= 1024 && equipment[j] - 1024 < Js5List.getConfigSize(Js5ConfigType.ITEM)) {
				int l1 = ItemDefinition.lookup(equipment[j] - 1024).team;
				if (l1 != 0)
					team = l1;
			}
		}
	...
	public ModelData getDialogueModel() {
		...
		for (int index = 0; index < 12; index++) {
			...
			if (appearanceId >= 256 && appearanceId < 1024 && !IdentityKit.lookup(appearanceId - 256).headLoaded()) {
				cached = true;
			}

			if (appearanceId >= 1024 && !ItemDefinition.lookup(appearanceId - 1024).isDialogueModelCached(myGender)) {
				cached = true;
			}
		}
		...
		ModelData headModels[] = new ModelData[12];
		...
		for (int modelIndex  = 0; modelIndex  < 12; modelIndex ++) {
			...
			if (appearanceId  >= 256 && appearanceId  < 1024) {

				ModelData subModel  = IdentityKit.lookup(appearanceId  - 256).headModel();
			...
			if (appearanceId  >= 1024) {
				ModelData subModel  = ItemDefinition.lookup(appearanceId  - 1024).getHeadPieceModel(myGender);
		...
		ModelData headModel = new ModelData(headModels,headModelsOffset);

		for (int index = 0; index < 5; index++) {
			if (appearanceColors[index] != 0) {
				int colorIdx = appearanceColors[index];
				if (colorIdx < Client.APPEARANCE_COLORS[index].length) {
					headModel.recolor((short) Client.APPEARANCE_COLORS[index][0], (short) Client.APPEARANCE_COLORS[index][colorIdx]);
				}
				if (index == 1 && colorIdx < Client.SHIRT_SECONDARY_COLORS.length) {
					headModel.recolor((short) Client.SHIRT_SECONDARY_COLORS[0], (short) Client.SHIRT_SECONDARY_COLORS[colorIdx]);
				}
			}
		}
	...
	public Model getAnimatedModel(SequenceDefinition var1, int var2, SequenceDefinition var3, int var4) {
		...
		if (super.sequence >= 0 && super.sequenceDelay == 0) {
			...
			k = animation.frameIds[super.sequenceFrame];
			if (super.movementSequence >= 0 && super.movementSequence != super.idleSequence)
				i1 = SequenceDefinition.get(super.movementSequence).frameIds[super.movementFrame];
			...
			k = SequenceDefinition.get(super.movementSequence).frameIds[super.movementFrame];
		...
		if (model_1 == null) {
			...
			for (int i2 = 0; i2 < 12; i2++) {
				...
				if (j1 >= 0 && i2 == 5)
					k2 = j1;
				if (k2 >= 256 && k2 < 1024 && !IdentityKit.lookup(k2 - 256).bodyCached())
					flag = true;
				if (k2 >= 1024 && !ItemDefinition.lookup(k2 - 1024).isEquippedModelCached(myGender))
					flag = true;
			}
		...
		if (model_1 == null) {
			ModelData aclass30_sub2_sub4_sub6s[] = new ModelData[12];
			int j2 = 0;

			// Debug: Log appearance info once when flag is set
			boolean shouldDebug = DEBUG_APPEARANCE && !debuggedOnce && this == Client.instance.localPlayer;
			if (shouldDebug) {
				debuggedOnce = true;
				System.out.println("========== PLAYER APPEARANCE DEBUG (gender=" + myGender + ") ==========");
			}
			...
			for (int l2 = 0; l2 < 12; l2++) {
				...
				if (i3 >= 256 && i3 < 1024) {
					int kitId = i3 - 256;
					IdentityKit kit = IdentityKit.lookup(kitId);
					ModelData model_3 = kit.getBody();

					if (shouldDebug) {
						String slotName = l2 < EQUIPMENT_SLOT_NAMES.length ? EQUIPMENT_SLOT_NAMES[l2] : "SLOT_" + l2;
						System.out.println("[SLOT " + l2 + " " + slotName + "] IDK=" + kitId +
							" bodyPartId=" + kit.bodyPartId +
							" model=" + (model_3 != null ? "OK" : "NULL"));
					}
				...
				if (i3 >= 1024) {
					ModelData model_4 = ItemDefinition.lookup(i3 - 1024)
			...
			if (shouldDebug) {
				System.out.println("========== END PLAYER APPEARANCE DEBUG ==========");
			}

			ModelData var20 = new ModelData(aclass30_sub2_sub4_sub6s,j2);
			for (int index = 0; index < 5; index++) {
				if (appearanceColors[index] != 0) {
					int colorIdx = appearanceColors[index];
					if (colorIdx < Client.APPEARANCE_COLORS[index].length) {
						var20.recolor((short) Client.APPEARANCE_COLORS[index][0], (short) Client.APPEARANCE_COLORS[index][colorIdx]);
					}
					if (index == 1 && colorIdx < Client.SHIRT_SECONDARY_COLORS.length) {
						var20.recolor((short) Client.SHIRT_SECONDARY_COLORS[0], (short) Client.SHIRT_SECONDARY_COLORS[colorIdx]);
					}
				}
			}
	...
	public boolean isHintArrowPointingAtPlayer(int index) {
		return index >= 0 && Client.instance.hintArrowType == 10 && Client.instance.npcIndices[index - Client.instance.playerCount] == Client.instance.hintArrowPlayerIndex;
	}
