    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        if (!handleInterfaceScrolling(e)) {
            if (mouseX > 0 && mouseX < 512 && MouseHandler.mouseY > Client.canvasHeight - 165
                ...
                /** MINIMAP ZOOM **/
                if (Client.instance.canZoomMap && Client.openInterfaceID == -1) {
                    int minimapCenterX = !Client.instance.isResized() ? 648 : Client.canvasWidth - 82;
                    int minimapCenterY = 84;
                    int dx = mouseX - minimapCenterX;
                    int dy = MouseHandler.mouseY - minimapCenterY;
                    if (dx * dx + dy * dy < 76 * 76) {
                        Client.minimapZoom += rotation * 25;
                        if (Client.minimapZoom < -200)
                            Client.minimapZoom = -200;
                        if (Client.minimapZoom > 500)
                            Client.minimapZoom = 500;
