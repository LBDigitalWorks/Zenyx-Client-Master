    ...
    public final synchronized void keyPressed(KeyEvent keyEvent) {
        ...
        if (!keyEvent.isConsumed())
        {
            ...
            if (keyEvent.isControlDown()) {
                ...
                switch (keyCode) {
                    case KeyEvent.VK_V:
                        String clipboardContents = Client.getClipboardContents();
                        // Handle login screen paste
                        if (!Client.loggedIn && Client.instance != null && Client.instance.loginScreen != null) {
                            // Filter to valid username/password characters
                            String filtered = clipboardContents.replaceAll("[^a-zA-Z0-9!@#$%^&*()_+=]", "");
                            if (Client.instance.loginScreen.getLoginScreenCursorPos() == 0) {
                                // Username field - max 12 chars
                                String newUsername = Client.instance.myUsername + filtered;
                                Client.instance.myUsername = newUsername.length() > 12 ? newUsername.substring(0, 12) : newUsername;
                            } else if (Client.instance.loginScreen.getLoginScreenCursorPos() == 1) {
                                // Password field - max 20 chars
                                String newPassword = Client.instance.myPassword + filtered;
                                Client.instance.myPassword = newPassword.length() > 20 ? newPassword.substring(0, 20) : newPassword;
                            }
                        } else {
                            String filtered = Client.sanitizeChatInput(clipboardContents);
                            if (!filtered.isEmpty()) {
                                String combined = Client.inputString + filtered;
                                int limit = Client.getChatInputLimit(combined);
                                int remaining = limit - Client.inputString.length();
                                if (remaining > 0) {
                                    if (filtered.length() > remaining) {
                                        filtered = filtered.substring(0, remaining);
                                    }
                                    Client.inputString += filtered;
            ...
            // Do not trigger tab keybinds while a text-input prompt is active or a widget input field is focused
            boolean isTypingInInput = (Client.instance.inputDialogState != 0) || Client.instance.isFieldInFocus();
            if (!isTypingInInput) {
                if (keyCode >= KeyEvent.VK_F1 && keyCode <= KeyEvent.VK_F12) {
                    Keybinding.isBound(keyCode);
                } else if (keyCode == KeyEvent.VK_ESCAPE) {
                    Keybinding.isBound(KeyEvent.VK_ESCAPE);
                    if (Configuration.escapeCloseInterface) {
