...
public class Bank extends RSInterface {
    public static final boolean BANK_DEBUG = false;
    public static final boolean BANK_SEARCH_DEBUG = true;
    ...
    public static void searchDebug(String message) {
        if (!BANK_SEARCH_DEBUG) {
            return;
        }
        String msg = "[BANK-SEARCH] " + message;
        System.out.println(msg);
        Client.instance.pushMessage(msg, 0, "");
    }
    ...
    private static int tabActionButtonId(int tab, int actionIndex) {
        int baseButtonId = TAB_SPRITE_DISPLAYS[tab];
        return baseButtonId + (baseButtonId - 58030) * 10 + actionIndex;
    }

    public static void openBankSearch() {
        searchDebug("openBankSearch currentTab=" + getCurrentBankTab() + " text='" + searchingBankString + "'");
    ...
    public static void closeBankSearch() {
        searchDebug("closeBankSearch text='" + searchingBankString + "'");
        interfaceCache[SEARCH_CONTAINER].resetItems();
    ...
    public static void handleButton(int buttonId) {
        if (BANK_DEBUG) {
            String msg = "[BANK-DBG] handleButton id=" + buttonId + ", currentTab=" + getCurrentBankTab();
            System.out.println(msg);
            Client.instance.pushMessage(msg, 0, "");
        }
        for (int index = 1; index < ITEM_CONTAINERS.length; index++) {
            if (buttonId == tabActionButtonId(index, 1) || buttonId == COLLAPSE_BUTTONS[index]) {
                if (BANK_DEBUG) {
                    String msg = "[BANK-DBG] collapse-tab action matched tab=" + index + " buttonId=" + buttonId;
                    System.out.println(msg);
                    Client.instance.pushMessage(msg, 0, "");
                }
        ...
        for (int tabIndex = 0; tabIndex < ITEM_CONTAINERS.length; tabIndex++) {
            int viewActionIndex = tabIndex == 0 ? 1 : 2;
            int priceCheckActionIndex = 0;
            if (buttonId == tabActionButtonId(tabIndex, priceCheckActionIndex)
                    || buttonId == tabActionButtonId(tabIndex, viewActionIndex)
                    || buttonId == OPEN_TAB_BUTTONS[tabIndex]) {
                if (BANK_DEBUG) {
                    String msg = "[BANK-DBG] open-tab action matched tab=" + tabIndex + " buttonId=" + buttonId;
                    System.out.println(msg);
                    Client.instance.pushMessage(msg, 0, "");
                }
                openBankTab(tabIndex);
            }
        }

        if (buttonId == SEARCH_BUTTON) {
            searchDebug("search button pressed isSearching=" + (bankScrollable.children[mainTabChildren[0]] == SEARCH_CONTAINER));
    ...
    public static void setupMainTab(RSInterface rsInterface, int x, int y) {
        if (rsInterface.id == SEARCH_CONTAINER) {
            ...
            searchDebug("populate results text='" + searchingBankString + "' results=" + resultCount);
    ...
    public static void openBankTab(int tab) {
        if (BANK_DEBUG) {
            String msg = "[BANK-DBG] openBankTab(" + tab + ")";
            System.out.println(msg);
            Client.instance.pushMessage(msg, 0, "");
        }
        closeBankSearch();
    ...
    private void editBank(TextDrawingArea[] tda) {
        ...
        RSInterface search = addInventoryContainer(SEARCH_CONTAINER, 10, BANK_CONTAINER_HEIGHT, 12, 0, true);
        search.actions = new String[] {"Withdraw 1", "Withdraw 5", "Withdraw 10", "Withdraw All", "Withdraw X", "Withdraw All but one"};
        search.contentType = 206;
        search.allowInvDraggingToOtherContainers = true;
    ...
    public void bank(TextDrawingArea[] tda) {
        ...
        addHoverButton(59000, "banktab/updated/search", 0, 24, 24, "Search", 250, 59004, 1);
        addHoveredButton(59004, "banktab/updated/search", 1, 24, 24, 59005);
        ...
        (interfaceCache[59000]).contentType = 0;
...
/*        setBounds(59014, 22, quantity_y, 20, rs);
        ...
        setBounds(59000, 386, 290, 48, rs);
        setBounds(59004, 386, 290, 49, rs);
        editBank(tda);
    }
