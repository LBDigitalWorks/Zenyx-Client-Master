...
public class SpawnContainer extends RSInterface {
    ...
    // === ONE-TIME SETUP (run once at client startup) ===
    private static final List<ItemDefEntry> ITEM_SEARCH_POOL = new ArrayList<>();
    static {
        for (int i = 0; i < 47590; i++) {
            ItemDefinition def = ItemDefinition.lookup(i);
            if (def != null && def.name != null && !def.name.isEmpty() && !def.name.contains("Dwarf remains")) {
                ITEM_SEARCH_POOL.add(new ItemDefEntry(i + 1, def.name.toLowerCase()));
            }
        }
    }
    ...
    private int containerHeight =  (int) Math.ceil(ITEM_SEARCH_POOL.size() / (double) containerWidth); // rows
    ...
    public void load() {
        ...
        scrollable.scrollMax = containerHeight * 32;
    ...
    // Tiny helper class to avoid repeated lookups or lowercase calls
    private static class ItemDefEntry {
        int id;
        String name;
        ItemDefEntry(int id, String name) {
            this.id = id;
            this.name = name;
        }
    }
    ...
    public void update(String message) {
        ...
        if (container == null || container.inventoryItemId == null || container.inventoryAmounts == null)
            return;

        message = message == null ? "" : message.toLowerCase().trim();

        int filled = 0;
        int maxSlots = Math.min(containerCount, container.inventoryItemId.length); // SAFETY CAP

        for (ItemDefEntry entry : ITEM_SEARCH_POOL) {
            if (message.isEmpty() || entry.name.contains(message)) {
                container.inventoryItemId[filled] = entry.id;
                container.inventoryAmounts[filled] = 1;
                filled++;
                if (filled >= maxSlots)
                    break;
            }
        ...
        // Clear remaining slots safely
        for (int i = filled; i < container.inventoryItemId.length; i++) {
