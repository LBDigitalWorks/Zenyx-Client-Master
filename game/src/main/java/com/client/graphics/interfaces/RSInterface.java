...
public class RSInterface {
	...
	public static void unpack(FileArchive streamLoader, TextDrawingArea textDrawingAreas[],
		...
		aMRUNodes_238 = new EvictingDualNodeHashTable(200000);
	...
	private static void fixBattleaxeSpecialBarSprites() {
		final int targetStart = 7499;
		final int targetEnd = 7511;
		final int sourceOffset = 75; // 7574-7586 mirrors 7499-7511

		for (int targetId = targetStart; targetId <= targetEnd; targetId++) {
			int sourceId = targetId + sourceOffset;
			if (targetId < 0 || sourceId < 0 || targetId >= interfaceCache.length || sourceId >= interfaceCache.length) {
				continue;
			}
			RSInterface target = interfaceCache[targetId];
			RSInterface source = interfaceCache[sourceId];
			if (target == null || source == null) {
				continue;
			}
			if (source.sprite1 != null) {
				target.sprite1 = source.sprite1;
			}
			if (source.sprite2 != null) {
				target.sprite2 = source.sprite2;
			}
			if (source.width > 0) {
	...
	public static void addButton(int id, Sprite sprite, String tooltip) {
		RSInterface tab = interfaceCache[id] = new RSInterface();
		tab.id = id;
		tab.parentID = id;
		tab.type = 5;
	...
	public static Model getModel(int model_type, int model_id, SequenceDefinition sequenceDefinition, int primary_index) {
		...
		if (model == null) {
			...
			switch (model_type) {
				case 1:
					var12 = ModelData.get(model_id,0);
					break;
				case 2:
	...
	public static final int TYPE_GRAND_EXCHANGE = 26;
	...
	public static void addProgressBar(int identity, int width, int height, int[] colorTypes) {
		...
		component.type = TYPE_PROGRESS_BAR_2021;
