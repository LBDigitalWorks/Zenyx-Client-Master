    ...
    public void onConfigChanged(int config, int value) {
        if (config == Configs.AUTOCAST_DEFENCE_CONFIG) {
            for (RSInterface autocast : autocasts) {
                RSInterface spriteInterface = get(getAutocastSpriteInterfaceId(autocast));
                if (spriteInterface == null) {
                    continue;
                }
                spriteInterface.sprite2 = value == 1 ? SPRITE_DEFENSIVE : SPRITE_STANDARD;
            }
            ...
            for (RSInterface autocast : autocasts) {
                ...
                spriteInterface.active = autocast.autocastSpellId == value;
            }
        }
    }
