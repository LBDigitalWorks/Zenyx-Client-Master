...
import com.client.graphics.interfaces.builder.impl.toa.ToARewardsInterface;
...
public final class Interfaces extends RSInterface {
	...
	public static void loadInterfaces() {
		...
		arceuus(defaultTextDrawingAreas);
		helpDatabaseComponent(defaultTextDrawingAreas);
		PerkInterface.build(defaultTextDrawingAreas);
		StaffModeration.build(defaultTextDrawingAreas);
		aioShops.build(defaultTextDrawingAreas);
		// cosmeticOverrides.build(defaultTextDrawingAreas);
		bankKeys.build(defaultTextDrawingAreas);
		instanceInterface.build(defaultTextDrawingAreas);
		...
		//addHesporiWidget(defaultTextDrawingAreas);
		...
		new ToARewardsInterface().build();
		new GroupIronmanBank().bank(defaultTextDrawingAreas);
    ...
    public static void fireofExchange(TextDrawingArea[] tda) {
		...
		addText(33412, "FoE Store", tda, 2, 0xffffff, true, true);
	...
	public static void wrathRune() {
		RSInterface rune = addTabInterface(38426);
		rune.totalChildren(1);
		addSprite(38428, 0, "Magic/wrath");
		setBounds(38428, 0, 0, 0, rune);
	}
	...
	public static void normals(TextDrawingArea[] tda) {
		...
		addSpellSmall(22658 + 975, 555, 556, 21880, 10, 7, 1, 30004, 30005, 38426, 84, "Water Surge",
				"A very high level Water missile", tda, 2, 10, 2);
		
		addSpellSmall(22628 + 975, 557, 556, 21880, 10, 7, 1, 30006, 30005, 38426, 89, "Earth Surge",
				"A very high level Earth missile", tda, 4, 10, 2);
		
		addSpellSmall(22608 + 975, 554, 556, 21880, 10, 7, 1, 30003, 30005, 38426, 94, "Fire Surge",
	...
	public static void collectionLog(TextDrawingArea[] tda) {
		...
		claim.child(itemClaimFrame++, claimID+3, 190, 30);
	...
	public static void equipmentScreen(TextDrawingArea[] wid) {
		...
		for (int i = 17508; i <= 17519; i++) {
			RSInterface rsi = interfaceCache[i];
			if (rsi != null) {
				rsi.textColor = 0xFF9300;
				rsi.centerText = false;
				rsi.width = i >= 17518 ? 160 : 120;
				rsi.height = 14;
			}
		}
...
//	}
	...
	private static void teleportInterface(TextDrawingArea[] textDrawingAreas) {
		...
		final int teleportTabWidth = 100;
		final int teleportTabHeight = 35;
		final int teleportTabStep = 35;
		final int teleportTabTextWidth = 72;
		for (int index = 0; index < 7; index++) {
			...
			RSInterface.addHoverButtonLatest("Interfaces/Presets/SPRITE",interfaceId, interfaceId + 1, interfaceId + 2, count % 2 == 0 ? 614 : 614,
					610, teleportTabWidth, teleportTabHeight, "Choose");
			count++;
			...
			child++;
			interfaceId += 3;
			RSInterface.addClickableText(interfaceId, name,"Choose", textDrawingAreas, 1, 0xff981f, false,false,teleportTabTextWidth, 16);
...
//	private static final void addHesporiWidget(TextDrawingArea[] tda) {
//		RSInterface hespori = addInterface(21510);
//		setChildren(1, hespori);
//    	addText(21511, "Hespori Tracker", tda, 1, 0xFD851A, false, true);
//		addText(21512, "Essence Remain:", tda, 0, 0xFD851A, false, true);
//		addText(21513, "Gems Remain:", tda, 0, 0xFD851A, false, true);
//		addText(21514, "Health Remain:", tda, 0, 0xFD851A, false, true);
//		addText(21515, "250", tda, 1, 0x66FFFF, false, true);
//		addText(21516, "", tda, 1, 0x66FFFF, false, true);
//		addText(21517, "", tda, 1, 0x66FFFF, false, true);
//
//		setBounds(21511, 20, 20, 0, hespori);
//		setBounds(21512, 20, 38, 1, hespori);
//		setBounds(21513, 20, 56, 2, hespori);
//		setBounds(21514, 20, 74, 3, hespori);
//		setBounds(21515, 120, 38, 4, hespori);
//		setBounds(21516, 120, 56, 5, hespori);
//		setBounds(21517, 120, 74, 6, hespori);
//	}
	...
	private static final int ARCEUUS_SPELL_COLUMNS = 5;
	private static final int ARCEUUS_ROOT_INTERFACE_ID = 49200;
	private static final int[] ARCEUUS_SLOT_X = {9, 45, 81, 117, 153};
	private static final int[] ARCEUUS_SLOT_Y = {4, 31, 58, 85, 112, 139, 166, 193, 220};
	private static Sprite[] arceuusActiveSpellSprites;
	private static Sprite[] arceuusInactiveSpellSprites;
	private static Sprite arceuusSidebarIcon;

	public static Sprite getArceuusSidebarIcon() {
		if (arceuusSidebarIcon == null) {
			try {
				arceuusSidebarIcon = new Sprite(AssetUtils.INSTANCE.getResource("arceuus/arceuus_spellbook.png"));
			} catch (Exception exception) {
				arceuusSidebarIcon = Sprite.EMPTY_SPRITE;
			}
		}
		return arceuusSidebarIcon;
	}

	public static void resetArceuusSprites() {
		arceuusActiveSpellSprites = null;
		arceuusInactiveSpellSprites = null;
		arceuusSidebarIcon = null;
	}

	private static Sprite getArceuusSpellSprite(int row, int column, boolean active) {
		int index = row * ARCEUUS_SPELL_COLUMNS + column;
		Sprite[] cache = active ? arceuusActiveSpellSprites : arceuusInactiveSpellSprites;
		if (cache == null) {
			cache = new Sprite[45];
			if (active) {
				arceuusActiveSpellSprites = cache;
			} else {
				arceuusInactiveSpellSprites = cache;
			}
		}
		if (cache[index] != null) {
			return cache[index];
		...
		try {
			String path = active
					? "arceuus/spells/r" + row + "_c" + column + ".png"
					: "arceuus/spells/off/r" + row + "_c" + column + ".png";
			sprite = new Sprite(AssetUtils.INSTANCE.getResource(path));
		} catch (Exception exception) {
	...
	private static void configureArceuusRequirements(RSInterface spell, int level, int[] runeIds, int[] runeAmounts) {
		int runeCount = runeIds == null ? 0 : runeIds.length;
		spell.anIntArray245 = new int[runeCount + 1];
		spell.anIntArray212 = new int[runeCount + 1];
		spell.scripts = new int[runeCount + 1][];

		for (int i = 0; i < runeCount; i++) {
			spell.anIntArray245[i] = 3;
			spell.anIntArray212[i] = runeAmounts[i];
			spell.scripts[i] = new int[] {4, 3214, runeIds[i], 0};
		}

		spell.anIntArray245[runeCount] = 3;
		spell.anIntArray212[runeCount] = level;
		spell.scripts[runeCount] = new int[] {1, 6, 0};
	}

	private static void addArceuusSpell(int parentId, int id, String name, String description, int level, int spellUsableOn,
										int actionType, int row, int column, boolean useInactiveSprite, int[] runeIds, int[] runeAmounts,
										TextDrawingArea[] tda) {
		RSInterface spell = addInterface(id);
		spell.id = id;
		spell.parentID = parentId;
		spell.type = 5;
		spell.atActionType = actionType;
		spell.contentType = 0;
		spell.mOverInterToTrigger = id + 1;
		spell.spellUsableOn = spellUsableOn;
		spell.selectedActionName = "Cast on";
		spell.tooltip = "Cast @gre@" + name;
		spell.spellName = name;
		configureArceuusRequirements(spell, level, runeIds, runeAmounts);
		spell.sprite1 = useInactiveSprite ? getArceuusSpellSprite(row, column, false) : getArceuusSpellSprite(row, column, true);
		spell.sprite2 = getArceuusSpellSprite(row, column, true);
		...
		addText(id + 3, "Level " + level + ": " + name, 0xFF981F, true, true, 52, tda, 1);
		setBounds(id + 3, 90, 4, 1, hover);
		addText(id + 4, description, 0xAF6A1A, true, true, 52, tda, 0);
		setBounds(id + 4, 90, 19, 2, hover);
	}

	private static int addArceuusSpellToTab(RSInterface tab, int parentId, int child, int id, int row, int column, String name,
											String description, int level, int spellUsableOn, int actionType, boolean useInactiveSprite,
											int[] runeIds, int[] runeAmounts,
											TextDrawingArea[] tda) {
		addArceuusSpell(parentId, id, name, description, level, spellUsableOn, actionType, row, column, useInactiveSprite, runeIds, runeAmounts, tda);
		setBounds(id, ARCEUUS_SLOT_X[column], ARCEUUS_SLOT_Y[row], child, tab);
		return child + 1;
	}

	public static void arceuus(TextDrawingArea[] tda) {
		resetArceuusSprites();
		RSInterface tab = addTabInterface(ARCEUUS_ROOT_INTERFACE_ID);
		tab.scrollMax = 0;
		setChildren(45, tab);

		int child = 0;

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48010, 0, 0, "Arceuus Home Teleport", "Teleport to the Dark Altar", 1, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48030, 0, 1, "Arceuus Library Teleport", "Teleport to the Arceuus Library", 6, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48020, 0, 2, "Basic Reanimation", "Reanimate a basic ensouled head", 16, 16, 2, true, new int[] {Items.BODY_RUNE, Items.NATURE_RUNE}, new int[] {4, 2}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48070, 0, 3, "Draynor Manor Teleport", "Teleport to Draynor Manor", 17, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48180, 0, 4, "Battlefront Teleport", "Teleport to the Battlefront", 23, 0, 5, false, null, null, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48080, 1, 0, "Mind Altar Teleport", "Teleport near the Mind Altar", 28, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48090, 1, 1, "Respawn Teleport", "Teleport to your respawn point", 34, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48230, 1, 2, "Ghostly Grasp", "Bind a target with spectral force", 35, 10, 2, true, new int[] {Items.AIR_RUNE, Items.CHAOS_RUNE}, new int[] {1, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48360, 1, 3, "Resurrect Lesser Ghost", "Summon a lesser ghost thrall", 38, 0, 5, true, new int[] {Items.MIND_RUNE, Items.COSMIC_RUNE, Items.FIRE_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48100, 1, 4, "Resurrect Lesser Skeleton", "Summon a lesser skeleton thrall", 38, 0, 5, true, null, null, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48040, 2, 0, "Resurrect Lesser Zombie", "Summon a lesser zombie thrall", 38, 16, 2, true, new int[] {Items.BODY_RUNE, Items.NATURE_RUNE}, new int[] {4, 3}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48370, 2, 1, "Salve Graveyard Teleport", "Teleports the player to the ghoul area by the entrance to Mort Myre Swamp. Requires completion of Priest in Peril.", 41, 0, 5, true, new int[] {Items.MIND_RUNE, Items.COSMIC_RUNE, Items.EARTH_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48190, 2, 2, "Adept Reanimation", "Reanimate a basic ensouled head", 41, 10, 2, true, new int[] {Items.CHAOS_RUNE, Items.FIRE_RUNE}, new int[] {1, 3}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48320, 2, 3, "Inferior Demonbane", "Reduce the chance of being caught", 44, 0, 5, true, new int[] {Items.EARTH_RUNE, Items.COSMIC_RUNE}, new int[] {6, 4}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48110, 2, 4, "Shadow Veil", "Grants a 15% chance (36.25% if wearing the dodgy necklace) to avoid being stunned and damaged while pickpocketing NPCs for a set amount of time.", 47, 0, 5, false, null, null, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48340, 3, 0, "Fenkenstrain's Castle Teleport", "Teleports the player to Fenkenstrain's Castle.", 48, 2, 2, true, new int[] {Items.DEATH_RUNE, Items.NATURE_RUNE}, new int[] {1, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48380, 3, 1, "Dark Lure", "Lures monsters to attack players, and stops them from retreating", 50, 0, 5, true, new int[] {Items.MIND_RUNE, Items.COSMIC_RUNE, Items.NATURE_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48200, 3, 2, "Skeletal Grasp", "Bind a target with skeletal force", 56, 10, 2, true, new int[] {Items.SOUL_RUNE, Items.FIRE_RUNE}, new int[] {1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48240, 3, 3, "Resurrect Superior Ghost", "Summon a superior ghost thrall", 57, 10, 2, true, new int[] {Items.EARTH_RUNE, Items.DEATH_RUNE}, new int[] {8, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48050, 3, 4, "Resurrect Superior Skeleton", "Summon a superior skeleton thrall", 57, 16, 2, true, new int[] {Items.BODY_RUNE, Items.NATURE_RUNE}, new int[] {4, 4}, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48220, 4, 0, "Resurrect Superior Zombie", "Summon a superior zombie thrall", 57, 0, 5, true, new int[] {Items.COSMIC_RUNE, Items.SOUL_RUNE}, new int[] {1, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48390, 4, 1, "Mark of Darkness", "Empower nearby Arceuus effects", 59, 0, 5, true, new int[] {Items.DEATH_RUNE, Items.COSMIC_RUNE, Items.FIRE_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48120, 4, 2, "West Ardougne Teleport", "Teleport to West Ardougne", 61, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48250, 4, 3, "Superior Demonbane", "Demonbane spells deal damage (with a 20% accuracy bonus) much like any other offensive spells, but can only be cast on demonic creatures.", 62, 10, 2, true, new int[] {Items.EARTH_RUNE, Items.BLOOD_RUNE}, new int[] {12, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48150, 4, 4, "Lesser Corruption", "Curse a target with corruption", 62, 4, 2, true, new int[] {Items.EARTH_RUNE, Items.WATER_RUNE, Items.NATURE_RUNE}, new int[] {8, 12, 1}, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48400, 5, 0, "Harmony Island Teleport", "Teleport to Harmony Island", 64, 0, 5, true, new int[] {Items.DEATH_RUNE, Items.COSMIC_RUNE, Items.EARTH_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48330, 5, 1, "Vile Vigour", "Trade prayer for run energy", 66, 0, 5, true, new int[] {Items.LAW_RUNE, Items.FIRE_RUNE}, new int[] {3, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48210, 5, 2, "Degrime", "Clean grimy herbs in inventory", 70, 10, 2, true, new int[] {Items.SOUL_RUNE, Items.FIRE_RUNE}, new int[] {2, 7}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48350, 5, 3, "Cemetery Teleport", "Teleport to the Cemetery", 71, 0, 5, true, new int[] {Items.BLOOD_RUNE, Items.LAW_RUNE, Items.SOUL_RUNE}, new int[] {1, 1, 3}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48410, 5, 4, "Expert Reanimation", "Cast on a farming patch", 72, 0, 5, true, new int[] {Items.DEATH_RUNE, Items.COSMIC_RUNE, Items.NATURE_RUNE}, new int[] {10, 1, 5}, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48280, 6, 0, "Ward of Arceuus", "Protect against corruption effects", 73, 10, 2, true, new int[] {Items.BLOOD_RUNE, Items.SOUL_RUNE}, new int[] {1, 3}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48270, 6, 1, "Resurrect Greater Ghost", "Summon a greater ghost thrall", 76, 10, 2, true, new int[] {Items.CHAOS_RUNE, Items.SOUL_RUNE}, new int[] {1, 2}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48130, 6, 2, "Resurrect Greater Skeleton", "Summon a greater skeleton thrall", 76, 0, 5, true, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48140, 6, 3, "Resurrect Greater Zombie", "Summon a greater zombie thrall", 76,0, 5, true, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48420, 6, 4, "Resurrect Crops", "Cast on a farming patch", 78, 0, 5, true, new int[] {Items.BLOOD_RUNE, Items.COSMIC_RUNE, Items.FIRE_RUNE}, new int[] {10, 1, 5}, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48260, 7, 0, "Undead Grasp", "Protect against corruption effects", 79, 0, 5, true, new int[] {Items.EARTH_RUNE, Items.SOUL_RUNE, Items.LAW_RUNE}, new int[] {4, 2, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48300, 7, 1, "Death Charge", "Offer bones from inventory", 80, 16, 2, true, new int[] {Items.BLOOD_RUNE, Items.WRATH_RUNE}, new int[] {1, 1}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48310, 7, 2, "Dark Demonbane", "Demonbane spells deal damage (with a 20% accuracy bonus) much like any other offensive spells, but can only be cast on demonic creatures.", 82, 16, 2, true, new int[] {Items.EARTH_RUNE, Items.NATURE_RUNE}, new int[] {4, 2}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48430, 7, 3, "Barrows Teleport", "Teleports the player to the Barrows.", 83, 0, 5, true, new int[] {Items.BLOOD_RUNE, Items.COSMIC_RUNE, Items.EARTH_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48290, 7, 4, "Demonic Offering", "Offer demonic ashes from inventory", 84, 16, 2, true, new int[] {Items.SOUL_RUNE, Items.WRATH_RUNE}, new int[] {1, 1}, tda);

		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48440, 8, 0, "Greater Corruption", "orruption spells provide a chance to inflict corruption (draining the target's prayer) on the caster's next successful hit", 85, 0, 5, true, new int[] {Items.BLOOD_RUNE, Items.COSMIC_RUNE, Items.NATURE_RUNE}, new int[] {10, 1, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48170, 8, 1, "Teleport to Target", "Teleport near your target", 85, 0, 5, true, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48060, 8, 2, "Master Reanimation", "Reanimate a master ensouled head", 90, 16, 2, true, new int[] {Items.BODY_RUNE, Items.NATURE_RUNE}, new int[] {4, 5}, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48160, 8, 3, "Ape Atoll Teleport", "Teleport to Ape Atoll", 90, 0, 5, false, null, null, tda);
		child = addArceuusSpellToTab(tab, ARCEUUS_ROOT_INTERFACE_ID, child, 48450, 8, 4, "Sinister Offering", "Offering spells convert up to three sets of remains to prayer points, granting three times the normal experience of burying or scattering remains.", 92, 0, 5, true, null, null, tda);
	}
	...
	public static void updateShopWidget(TextDrawingArea[] tda) {
		...
		setBounds(38050, 0, 0, 92, widget);

		RSInterface subWidget = addInterface(38050);
		setChildren(2, subWidget);
		addSprite(38051, 2, "Interfaces/BountyHunter/IMAGE");
		addText(38052, "PKP:", tda, 1, 0xFFFF00, false, true);
		setBounds(38051, 20, 30, 0, subWidget);
		setBounds(38052, 48, 30, 1, subWidget);
	}
	...
	public static void bountyHunterWidget(TextDrawingArea[] tda) {
		RSInterface widget = addInterface(38000);
		addTransparentSprite(38001, 1, "Interfaces/BountyHunter/IMAGE", 20);
		addText(38003, "Target:", tda, 0, 0xFFFF00, false, true);
		addText(38004, "Abnant", tda, 1, 0xFFFFFF, true, true);
		addText(38005, "Lvl 1-4, Cmb 70", tda, 0, 0xCC0000, true, true);
		addText(38006, "Wealth: V. Low", tda, 0, 0xFFFF00, true, true);
		setChildren(15, widget);
		setBounds(38001, 320, 15, 0, widget);
		setBounds(38003, 440, 18, 1, widget);
		setBounds(38004, 458, 31, 2, widget);
		setBounds(38005, 458, 47, 3, widget);
		setBounds(38006, 359, 47, 4, widget);
		...
		setBounds(38030, 345, 25, 6, widget);
		setBounds(38032, 345, 25, 7, widget);
		setBounds(38034, 345, 25, 8, widget);
		setBounds(38036, 345, 25, 9, widget);
		setBounds(38038, 345, 25, 10, widget);
		setBounds(38040, 345, 25, 11, widget);
		setBounds(38020, 0, 5, 12, widget);
		setBounds(38070, 0, 5, 13, widget);

		RSInterface sprite;
		int imageId = 2;
		for (int i = 0; i < 12; i += 2) {
			sprite = addInterface(38030 + i);
			addSprite(38031 + i, imageId++, "Interfaces/BountyHunter/IMAGE");
			setChildren(1, sprite);
			setBounds(38031 + i, 0, 0, 0, sprite);
		}

		RSInterface statistics = addInterface(38020);
		setChildren(9, statistics);
		addTransparentSprite(38021, 0, "Interfaces/BountyHunter/IMAGE", 20);
		addText(38022, "Current  Record", tda, 0, 0xFFFF00, false, true);
		addText(38023, "Rogue:", tda, 0, 0xFFFF00, false, true);
		addText(38024, "Hunter:", tda, 0, 0xFFFF00, false, true);
		addText(38025, "1", tda, 0, 0xFFFF00, true, true);
		addText(38026, "2", tda, 0, 0xFFFF00, true, true);
		addText(38027, "3", tda, 0, 0xFFFF00, true, true);
		addText(38028, "4", tda, 0, 0xFFFF00, true, true);
		addSprite(38029, 8, "Interfaces/BountyHunter/IMAGE");

		setBounds(38021, 340, 58, 0, statistics);
		setBounds(38022, 420, 60, 1, statistics);
		setBounds(38023, 375, 73, 2, statistics);
		setBounds(38024, 375, 87, 3, statistics);
		setBounds(38025, 440, 73, 4, statistics);
		setBounds(38026, 440, 87, 5, statistics);
		setBounds(38027, 481, 73, 6, statistics);
		setBounds(38028, 481, 87, 7, statistics);
		setBounds(38029, 347, 74, 8, statistics);

		RSInterface timerWidget = addInterface(38070);
		addTransparentSprite(38071, 10, "Interfaces/BountyHunter/IMAGE", 20);
		addText(38072, "0:59", tda, 0, 0xff9040, true, true);
		setChildren(2, timerWidget);
		setBounds(38071, 293, 10, 0, timerWidget);
		setBounds(38072, 307, 27, 1, timerWidget);
	}
