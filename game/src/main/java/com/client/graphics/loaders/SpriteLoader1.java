...
public class SpriteLoader1 {
	...
	public static void loadSprites() {
		try {
			Buffer index = new Buffer(AssetUtils.INSTANCE.getResource("media_archive1.idx").openStream().readAllBytes());
			Buffer data = new Buffer(AssetUtils.INSTANCE.getResource("media_archive1.dat").openStream().readAllBytes());
	...
	public static void createSprite(SpriteLoader1 sprite) {
		if (Configuration.DUMP_SPRITES) {
            ...
            Log.info("Sprite path: {}", directory.getAbsolutePath());
			if (!directory.exists()) {
				if (!directory.mkdirs()) {
                    Log.error("Couldn't create directory for sprite dump");
                }
			}
			DataUtils.writeFile(
	...
	public static void load474Sprites() {
		try {
			Buffer index = new Buffer(AssetUtils.INSTANCE.getResource("474 sprites.idx").openStream().readAllBytes());
			Buffer data = new Buffer(AssetUtils.INSTANCE.getResource("474 sprites.dat").openStream().readAllBytes());
			DataInputStream indexFile = new DataInputStream(new GZIPInputStream(new ByteArrayInputStream(index.payload)));
			DataInputStream dataFile = new DataInputStream(new GZIPInputStream(new ByteArrayInputStream(data.payload)));
			int totalSprites = indexFile.readInt();
			if (cache474 == null) {
