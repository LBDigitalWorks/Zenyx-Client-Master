...
public class Js5System {
    private static final Logger log = LoggerFactory.getLogger(Js5System.class);
    ...
    public static void connect() {
        if (Client.jagexNetThread.crcMissmatches >= 4) {
            ...
            log.warn("CRC mismatches exceeded 4, setting game state to 1000");
        } else {
            if (Client.jagexNetThread.ioExceptions >= 4) {
                if (Client.instance.gameState <= 5) {
                    Client.instance.error("js5io");
                    Client.instance.gameState = 1000;
                    log.warn("IO exceptions exceeded 4 and game state <= 5, setting game state to 1000");
                    return;
                }
            ...
            if (--js5Cycles + 1 <= 0) {
                try {
                    if (js5ConnectState == 0) {
                        js5SocketTask = GameEngine.taskHandler.newSocketTask(Configuration.DEDICATED_SERVER_ADDRESS, Configuration.SERVICES_PORT);
                        ++js5ConnectState;
                        log.info("js5ConnectState set to 1, creating socket task");
                    }

                    if (js5ConnectState == 1) {
                        if (js5SocketTask.status == 2) {
                            js5Error(-1);
                            log.info("Socket task status is 2, reporting an error");
                            return;
                        }

                        if (js5SocketTask.status == 1) {
                            ++js5ConnectState;
                            log.info("Socket task status is 1, advancing to js5ConnectState 2");
                        }
                    }
                    ...
                    if (js5ConnectState == 2) {
                        ...
                        log.info("js5ConnectState set to 2, creating BufferedNetSocket and sending data");
                    }
                    ...
                    if (js5ConnectState == 3) {
                        if (js5Socket.available() > 0) {
                            ...
                            if (socket != 0) {
                                js5Error(socket);
                                log.error("Received non-zero socket value, reporting an error");
                                return;
                            }

                            ++js5ConnectState;
                            log.info("js5ConnectState advanced to 3");
                        } else if (MonotonicClock.currentTimeMillis() - FileSystem.currentTimeMills > 30000L) {
                            js5Error(-2);
                            log.error("Socket not available, and time exceeded 30000ms, reporting an error");
                            return;
                        }
                    }
                    ...
                    if (js5ConnectState == 4) {
                        ...
                        js5ConnectState = 0;
                        js5Errors = 0;
                        log.info("js5ConnectState set to 4, performing final initialization");
                    }
                } catch (IOException exception) {
                    js5Error(-3);
                    log.error("IOException caught, reporting an error");
                }
            }
        }
    }
