...
//		} else {
	...
	public static void loadManifest() {
		...
		try {
			URL resource = AssetUtils.INSTANCE.getResource("spriteMappings.json");
			if (resource == null) {
				System.err.println("[Sprite] Manifest not found: spriteMappings.json");
				mappedSprites = Collections.emptyMap();
				return;
			}

			try (Reader reader = new InputStreamReader(resource.openStream(), StandardCharsets.UTF_8)) {
				mappedSprites = new Gson().fromJson(reader, type);
				if (mappedSprites == null) {
					System.err.println("[Sprite] Manifest was empty or malformed.");
					mappedSprites = Collections.emptyMap();
				}
			}
		} catch (Exception e) {
			System.err.println("[Sprite] Failed to load sprite manifest: " + e.getMessage());
			e.printStackTrace();
			mappedSprites = Collections.emptyMap();
		}
	}

	public static ArrayList<String> beingUsed = new ArrayList<>();

	public Sprite(String img) {
		if (img == null || img.isEmpty()) {
			System.err.println("[Sprite] Invalid image name (null or empty)");
			return;
		}

		try {
			// Normalize the key name
			String imgName = img.startsWith("/") ? img.substring(1) : img;
			imgName = imgName.toLowerCase();

			// Ensure manifest is loaded
			if (mappedSprites == null || mappedSprites.isEmpty()) {
				System.err.println("[Sprite] Manifest not loaded. Loading now...");
				loadManifest();
			}

			Integer spriteId = mappedSprites.get(imgName);
			if (spriteId == null) {
				//System.err.println("[Sprite] Sprite not mapped: " + imgName);
				return;
			}

			var cached = ImageCache.get(spriteId);
			if (cached == null) {
				System.err.println("[Sprite] Missing ImageCache entry for ID: " + spriteId + " (" + imgName + ")");
				return;
			}

			Image image = cached.toBufferedImage();
			if (image == null) {
				System.err.println("[Sprite] Failed to convert image to BufferedImage for: " + imgName);
				return;
			}
			...
			System.err.println("[Sprite] Pixel grab interrupted for: " + img);
		} catch (Exception e) {
			System.err.println("[Sprite] Failed to load sprite: " + img + " - " + e.getMessage());
			e.printStackTrace();
		}
	}
