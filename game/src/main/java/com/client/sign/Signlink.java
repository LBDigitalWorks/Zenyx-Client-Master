...
public class Signlink {
    ...
    private static final Logger log = LoggerFactory.getLogger(Signlink.class);
	...
	public static File getCachedFile(String cacheType, String cacheKey, int cacheVersion) {
		File overrideCacheDir = getOverrideCacheDir();
	...
	public static final String getCacheDirectory() {
		File overrideCacheDir = getOverrideCacheDir();
		if (overrideCacheDir != null) {
			return overrideCacheDir.getAbsolutePath() + "/";
		}
	...
	private static File getOverrideCacheDir() {
		String override = System.getProperty(CACHE_DIR_OVERRIDE_PROP);
		if (override == null || override.isBlank()) {
			override = System.getenv(CACHE_DIR_OVERRIDE_ENV);
		}
		if (override != null && !override.isBlank()) {
			return new File(override);
		}
		File localServerCache = new File(LOCAL_SERVER_CACHE_DIR);
		if (localServerCache.exists()) {
