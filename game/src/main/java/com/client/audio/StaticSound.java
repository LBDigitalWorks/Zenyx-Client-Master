...
public class StaticSound {
	...
	public static void playAnimationSound(SequenceDefinition var0, int var1, int var2, int var3, Entity var4) {
		if (soundEffectCount < 50 && var0.getSkeletalSounds() != null && var0.getSkeletalSounds().containsKey(var1)) {
				ArrayList var5 = (ArrayList) var0.skeletalSounds.get(var1);
				if (!var5.isEmpty()) {
					int var8 = 0;
					int var10;
					if (var5.size() > 1) {
						...
						for (Iterator var11 = var5.iterator(); var11.hasNext(); ++var8) {
							SoundData var12 = (SoundData)var11.next();
							int var13 = var10;
						...
						if ((var10 <= 0 || Client.instance.settingManager.getAreaSoundEffectVolume() != 0) && (var10 != 0 || Client.instance.settingManager.getSoundEffectVolume() != 0)) {
							method520(var12, var2, var3, var4 == Client.localPlayer);
						}
					}
					}
				}
	...
	static void method520(SoundData var0, int var1, int var2, boolean var3) {
		if (var0 != null) {
			if (var0.field2298 == 0) {
				...
				soundLocations[soundEffectCount] = (var5 << 8) + var0.field2298 + (var4 << 16);
			}

			soundEffectIds[soundEffectCount] = var0.field2294;
			queuedSoundEffectLoops[soundEffectCount] = var0.field2296;
			queuedSoundEffectDelays[soundEffectCount] = 0;
			soundEffects[soundEffectCount] = null;
			field609[soundEffectCount] = var0.field2294;
			++soundEffectCount;
		}
	}

	public static void playSkeletalSounds(SequenceDefinition var0, int var1, int var2, int var3, Entity var4) {
		if (soundEffectCount < 50 && var0.skeletalSounds != null && var0.skeletalSounds.containsKey(var1)) {
				ArrayList var7 = (ArrayList)var0.skeletalSounds.get(var1);
				if (!var7.isEmpty()) {
					int var8 = 0;
					int var10;
					if (var7.size() > 1) {
					...
					SoundData var5 = (SoundData) var7.get(var8);
					if (var5 != null) {
						var10 = var5.field2295 & 31;
						if ((var10 <= 0 || Client.instance.settingManager.getAreaSoundEffectVolume() != 0) && (var10 != 0 || Client.instance.settingManager.getSoundEffectVolume() != 0) && var5 != null) {
							if (var5.field2295 == 0) {
	...
	public static void queueAreaSoundEffect(int soundId, int worldX, int worldY, int rangeTiles, int delay) {
		if (Client.instance.settingManager.getAreaSoundEffectVolume() == 0 || soundEffectCount >= 50) {
			return;
		}

		int tileX = (worldX - 64) / 128;
		int tileY = (worldY - 64) / 128;
		int range = Math.max(1, Math.min(rangeTiles, 255));

		soundEffectIds[soundEffectCount] = soundId;
		queuedSoundEffectLoops[soundEffectCount] = 1;
		queuedSoundEffectDelays[soundEffectCount] = delay;
		soundEffects[soundEffectCount] = null;
		soundLocations[soundEffectCount] = (tileX << 16) + (tileY << 8) + range;
		field609[soundEffectCount] = 1;
		++soundEffectCount;
	}
