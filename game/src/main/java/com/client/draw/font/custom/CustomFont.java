...
public final class CustomFont {
    ...
    private void drawPixels(final byte[] pixels, final int width, final int height, int xPos, int yPos,
        ...
        int w = width;
        int h = height;
        int tmp = Rasterizer2D.Rasterizer2D_xClipStart;
        if (xPos < tmp) {
            x += tmp - xPos;
            w -= tmp - xPos;
            xPos = tmp;
        }
        tmp = Rasterizer2D.Rasterizer2D_yClipStart;
        if (yPos < tmp) {
            y += tmp - yPos;
            h -= tmp - yPos;
            yPos = tmp;
        }
        tmp = Rasterizer2D.Rasterizer2D_xClipEnd;
        if (xPos + w > tmp)
            w = tmp - xPos;

        tmp = Rasterizer2D.Rasterizer2D_yClipEnd;
        if (yPos + h > tmp)
            h = tmp - yPos;
        ...
        tmp = Rasterizer2D.Rasterizer2D_width;
        int localIndex = x + y * width;
        int globalIndex = xPos + yPos * tmp;
        int[] dst = Rasterizer2D.Rasterizer2D_pixels;
