...
public abstract class AbstractFont extends Rasterizer2D {
    ...
    static void drawGlyph2(byte[] var0, int x, int y, int width, int height, int color) {
        int destinationIndex = x + y * Rasterizer2D.Rasterizer2D_width;
        int destinationStep = Rasterizer2D.Rasterizer2D_width - width;
        int sourceIndex = 0;
        int sourceSkip = 0;
        int clip;
        if (y < Rasterizer2D.Rasterizer2D_yClipStart) {
            clip = Rasterizer2D.Rasterizer2D_yClipStart - y;
            height -= clip;
            y = Rasterizer2D.Rasterizer2D_yClipStart;
            sourceSkip += width * clip;
            destinationIndex += clip * Rasterizer2D.Rasterizer2D_width;
        }
        ...
        if (x < Rasterizer2D.Rasterizer2D_xClipStart) {
            clip = Rasterizer2D.Rasterizer2D_xClipStart - x;
            width -= clip;
        ...
        if (width + x > Rasterizer2D.Rasterizer2D_xClipEnd) {
            clip = width + x - Rasterizer2D.Rasterizer2D_xClipEnd;
            width -= clip;
            sourceIndex += clip;
            destinationStep += clip;
        }
        ...
        if (width > 0 && height > 0) {
            placeGlyph(Rasterizer2D.Rasterizer2D_pixels, var0, color, sourceSkip, destinationIndex, width, height, destinationStep, sourceIndex);
        }
    }
    ...
    static void drawGlyphAlpha2(byte[] glyph, int x, int y, int width, int height, int color, int shadow) {
        ...
        if (width > 0 && height > 0) {
            placeGlyphAlpha(Rasterizer2D.Rasterizer2D_pixels, glyph, color, sourceSkip, destinationIndex, width, height, destinationStep, sourceIndex, shadow);
        }
    }
