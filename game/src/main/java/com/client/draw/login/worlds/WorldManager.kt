...
object WorldManager {
    ...
    fun renderWorldSelect() {
        ...
        worldList.forEachIndexed { index, world ->
            ...
            ImageCache.get(world.spriteId).drawSprite(worldButtonX, worldButtonY)

            if(Client.instance.newclickInRegion(worldButtonX,worldButtonY,ImageCache.get(world.spriteId))) {
                Client.server = world.ip
                Client.instance.loginScreen.loginState = LoginState.LOGIN
            }

            if (Client.instance.newmouseInRegion(worldButtonX, worldButtonY, ImageCache.get(world.spriteId))) {
                drawTooltip(world)
            }
