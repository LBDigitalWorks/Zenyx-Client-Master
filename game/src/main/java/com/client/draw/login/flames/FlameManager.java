    ...
    final void drawFlames(int xPosition, int yPosition, int alpha) {
        ...
        for (int offset = 1; offset < 255; ++offset) {
            ...
            for (int xPixel = clippingStart; xPixel < clippingEnd; ++xPixel) {
                ...
                int currentColumn = rowStart % Rasterizer2D.Rasterizer2D_width;

                if (currentStrength != 0 && currentColumn >= Rasterizer2D.Rasterizer2D_xClipStart && currentColumn < Rasterizer2D.Rasterizer2D_xClipEnd) {
